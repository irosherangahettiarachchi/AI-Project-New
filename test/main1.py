...
def manager_agent(state: AgentState):
    print("--- [7/6] Manager Agent ---")
    llm = get_llm("manager") # Use the manager LLM for oversight
    
    summary_prompt = f"""
    Review all outputs and provide high-level recommendations.
    Include any flagged issues, critical updates, or alerts.
    
    Current State Summary:
    - SKUs Selected: {len(state['selected_skus'])}
    - Listings: {len(state['listings'])}
    - QA Failures: {len(state['listing_redlines'])}
    - Price Updates: {len(state['price_updates'])}
    - Stock Updates: {len(state['stock_updates'])}
